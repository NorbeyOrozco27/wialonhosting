// api/webhook.ts - VERSIÓN CORREGIDA (5 ARGUMENTOS)
...
export default async function handler(req: any, res: any) {
  ...
  try {
    const { data: vehiculos } = await supabaseA.from('Vehículos').select('id').eq('numero_interno', unitClean).single();
    if (!vehiculos) return res.json({ msg: "Bus no encontrado" });

    const { data: turnos } = await supabaseA.from('operacion_diaria').select('horario_id').eq('fecha', hoyCol).eq('vehiculo_id', vehiculos.id);
    if (!turnos || turnos.length === 0) return res.json({ msg: "Sin turnos" });

    const idsHorarios = turnos.map(t => t.horario_id);
    const { data: infoHorarios } = await supabaseA.from('Horarios').select('id, hora, destino, origen').in('id', idsHorarios);
    ...
    if (infoHorarios) {
        for (const h of infoHorarios) {
            // AQUÍ ESTÁ EL CAMBIO CRÍTICO: AGREGAR EL 5to ARGUMENTO ""
